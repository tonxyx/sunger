if [[ ! -d /root/.ssh ]]; then
  ...
  mkdir /root/.ssh && touch /root/.ssh/known_hosts && ssh-keyscan -H github.com >> /root/.ssh/known_hosts && chmod 600 /root/.ssh/known_hosts
    ...
    echo "create database sunger" | mysql -uroot -ppassword

    rm /etc/nginx/sites-enabled/default
    ln -s /vagrant/Vagrant/nginx/sunger /etc/nginx/sites-enabled/sunger
    ln -s /vagrant/Vagrant/nginx/sunger /etc/nginx/sites-available/sunger
